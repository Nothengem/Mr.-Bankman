...
export var road_life_stage = 1 #1 дорога появилась, 2 дорога активная, 3 дорога должна уйти
...
func road_life_stage():
	if road_life_stage == 1:
		$AnimationPlayer.play("RoadAppera")
		road_life_stage = 2
		is_active = true
		is_active()
	elif road_life_stage == 2:
		$AnimationPlayer.play("RoadBehind")
		road_life_stage = 3
		Scriptwriter.CardDeliveryinRunning = all_dangers
	elif road_life_stage == 3:
		$AnimationPlayer.play("RoadBehind")


func _on_AnimationPlayer_animation_finished(RoadBehind):
	get_tree().call_group("ChooseRoadButton", "show_buttons")
	
	if road_life_stage == 3:
		queue_free()



func _on_AnimationPlayer2_animation_finished(RoadAppera):
	is_active =  true
