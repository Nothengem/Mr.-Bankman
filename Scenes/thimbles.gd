...
#переменная отвечающая за то, закроет ли татч баттон экран или нет
...
func cards_mix_generate():
	...
	mini_cards.shuffle()
	first_card = mini_cards[0]
	second_card = mini_cards[1]
...
func _on_Timer_start_final_anim_timeout():
	...
	Twee.start()
	Twee.interpolate_property(choosen_mini_card, "scale", choosen_mini_card.scale, Vector2(2.5, 2.5), 0.5, Tween.TRANS_LINEAR, Tween.EASE_IN)
	Twee.start()
	$Timer_fade.start()
